...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        ##            os.rename(input_file, input_file.split('.')[0] + '_' + 'pep.fasta')
            if '\\' in input_file:
                index1 = re.finditer('\.', input_file)
                index2 = re.finditer(r"\\", input_file)
                dup_item1 = [m.start(0) for m in index1]
                dup_item2 = [m.start(0) for m in index2]
                print dup_item2
                file_name_op = input_file[dup_item2[-1] + 1:dup_item1[-1]]
                file_name_op1 = input_file[:dup_item1[-1]] + '_pep.' + sys.argv[1][dup_item1[-1] + 1:]
                file_name_op2 = input_file[:dup_item2[-1]]
                #file_name_op = input_file.split('.')[0]
                input_file = file_name_op + '_pep.' + input_file[dup_item1[-1] + 1:len(input_file)]
            else:
                file_name_op = input_file.split('.')[0]
                input_file = input_file.split('.')[0] + '_pep.' + input_file.split('.')[1]
            print "Checking the database for redundancy \n"
            print file_name_op2 + '/' + file_name_op + '_s2s.fasta'
            s2s_file = open(file_name_op2 + '/' + file_name_op + '_s2s.fasta', 'w') # creating the stop to stop sequence fasta file
            m2s_file = open(file_name_op2 + '/' + file_name_op + '_m2s.fasta', 'w') # creating Met to stop sequecne fasta file
            combined_file = open(file_name_op2 + '/' + file_name_op + '_combined.fasta', 'w') # creating the combined(stop to stop and Met to stop sequence) fasta file
            dicts = {}

            print file_name_op1
            #removing the redundancy
            read_fasta_file = read_fasta(file_name_op1)
