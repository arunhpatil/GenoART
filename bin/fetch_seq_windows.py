...
def no_gtf(infile):
    ...
    read_file_no_gtf = read_fasta(infile)
    for no_gtf_raw in read_file_no_gtf:
        if '|' in no_gtf_raw[0].rstrip():
            lst_chromosomes.append(no_gtf_raw[0].rstrip())
        elif ' ' in no_gtf_raw[0]:
            lst_chromosomes.append(no_gtf_raw[0].rstrip())
        else:
            lst_chromosomes.append(no_gtf_raw[0].rstrip())
    print "The annotation file contains the following chromosomes/scaffolds:\n", lst_chromosomes
    index1 = re.finditer('\.', sys.argv[1])
    dup_item1 = [m.start(0) for m in index1]
    write_file = open(file_name_op + '_op.' + sys.argv[1][dup_item1[-1] + 1:] , 'w')
    read_file = read_fasta(infile)
    write_file = open(file_name_op + '_op.' + infile[dup_item1[-1] + 1:], 'w')
    read_file = read_fasta(infile)
    ...
    if len(sys.argv) > 1:
        try:
            ...
            if input_gtf != '0':
                ...
                index1 = re.finditer('\.', load_fasta_file)
                dup_item1 = [m.start(0) for m in index1]
                write_file1 = open(file_name_op + '_op.' + load_fasta_file[dup_item1[-1] + 1:], 'w')
            ...
            print "Please check Error.log file for more detail"
